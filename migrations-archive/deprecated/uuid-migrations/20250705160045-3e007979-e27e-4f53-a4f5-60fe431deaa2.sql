-- ===============================================================
-- ACHIEVEMENT SYSTEM MIGRATION - FINAL VERSION
-- ===============================================================
-- This SQL script fixes and enhances the achievement system:
--
-- 1. Fixes the ambiguous column reference issue
-- 2. Adds null-safety checks for award IDs
-- 3. Improves logging and error reporting
-- 4. Adds support for more achievement types
-- 5. Properly recreates the trigger
--
-- Last updated: July 3, 2025
-- ===============================================================

-- First, drop existing triggers and functions safely
...
BEGIN
  ...
  -- Check First Submission / تولد یک کدنویس
  IF exercise_count >= 1 THEN
    ...
    IF award_id_var IS NOT NULL THEN
      ...
      END IF;
    ELSE
      result_message := 'Warning: Could not find award with name ''تولد یک کدنویس'' or ''First Submission''';
      RETURN NEXT result_message;
    END IF;
  END IF;

  -- Check Perfect Score / استاد کامل
  ...
  IF perfect_scores >= 1 THEN
    SELECT awards.id, awards.name INTO award_id_var, award_name 
    FROM awards 
    WHERE awards.name = 'استاد کامل' OR awards.name = 'Perfect Score';
    ...
    IF award_id_var IS NOT NULL THEN
      ...
      END IF;
    ELSE
      result_message := 'Warning: Could not find award with name ''استاد کامل'' or ''Perfect Score''';
      RETURN NEXT result_message;
    END IF;
  END IF;

  -- Check High Achiever / نابغه برنامه‌نویسی (90%+ on 5 exercises)
  ...
  IF high_scores >= 5 THEN
    SELECT awards.id, awards.name INTO award_id_var, award_name 
    FROM awards 
    WHERE awards.name = 'نابغه برنامه‌نویسی' OR awards.name = 'High Achiever';
    ...
    IF award_id_var IS NOT NULL THEN
      ...
      END IF;
    ELSE
      result_message := 'Warning: Could not find award with name ''نابغه برنامه‌نویسی'' or ''High Achiever''';
      RETURN NEXT result_message;
    END IF;
  END IF;
  ...
  -- Check Early Bird / تکمیل‌کننده زودهنگام (submit 5 exercises before due date)
  ...
  IF early_submissions >= 5 THEN
    SELECT awards.id, awards.name INTO award_id_var, award_name 
    FROM awards 
    WHERE awards.name = 'تکمیل‌کننده زودهنگام' OR awards.name = 'Early Bird';
    ...
    IF award_id_var IS NOT NULL THEN
      ...
      END IF;
    ELSE
      result_message := 'Warning: Could not find award with name ''تکمیل‌کننده زودهنگام'' or ''Early Bird''';
      RETURN NEXT result_message;
    END IF;
  END IF;

  -- Check Exercise Enthusiast (25 exercises) - This may have a Persian equivalent
...
COMMENT ON FUNCTION public.trigger_check_achievements() IS 'Trigger function that calls check_and_award_achievements after exercise submissions are inserted or updated';
