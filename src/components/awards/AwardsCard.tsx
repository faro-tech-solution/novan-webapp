        ...
        {studentAwards.length === 0 ? (
          ...
          <div className="space-y-4">
            {studentAwards.slice(0, 5).map((studentAward) => (
              <div key={studentAward.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                <div className="flex items-center space-x-3 space-x-reverse">
                  <div className="flex-shrink-0">
                    {getRarityIcon(studentAward.awards.rarity)}
                  </div>
                  <div>
                    <h4 className="font-medium">{studentAward.awards.name}</h4>
                    <p className="text-sm text-gray-600">{studentAward.awards.description}</p>
                    <p className="text-xs text-gray-500">کسب شده در: {formatDate(studentAward.earned_at)}</p>
                  </div>
                </div>
