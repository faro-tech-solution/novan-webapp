...
# Run both the achievement function enhancement and trigger fix migrations
echo "Running fixed achievement function and trigger migrations..."
...
echo "First dropping all existing achievement functions to avoid dependency issues..."
psql -f migrations/drop_all_achievement_functions.sql

echo "Running migration: enhance_achievement_function_null_safe.sql"
psql -f migrations/enhance_achievement_function_null_safe.sql

echo "Running migration: fix_trigger_function.sql"
psql -f migrations/fix_trigger_function.sql

echo "Checking awards table to ensure all required awards exist..."
psql -f migrations/check_awards_table.sql

echo "Achievement function and trigger migrations completed."
